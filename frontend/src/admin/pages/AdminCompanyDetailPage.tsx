...
export function AdminCompanyDetailPage() {
  ...
  return (
    <div className="space-y-6">
      <button onClick={() => navigate("/admin/companies")} className="flex items-center gap-2 text-sm font-bold text-violet-300 hover:text-white">
        <ArrowLeft size={17} />
        Retour entreprises
      </button>
      ...
      <div className="rounded-xl border border-white/10 bg-white/5 p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs font-bold uppercase tracking-wider text-violet-400">Tenant detail</p>
          ...
          <div className="mt-4 space-y-2">
            {data?.users.map((user) => (
              <article key={user.id} className="flex items-center justify-between gap-3 rounded-lg border border-white/5 bg-white/5 p-3">
                ...
                <span className="rounded-full bg-violet-500/20 px-2 py-1 text-xs font-bold text-violet-200">{user.account_status}</span>
              </article>
            ))}
          </div>
        </section>
          ...
          <div className="mt-4 space-y-2">
            {data?.alerts.length ? data.alerts.map((alert) => (
              <article key={alert.id} className="flex items-center gap-3 rounded-lg border border-white/5 bg-white/5 p-3">
                <span className={`grid h-10 w-10 place-items-center rounded-lg ${alert.severity === "high" ? "bg-rose-500/20 text-rose-200" : "bg-amber-500/20 text-amber-200"}`}>
