...
const QUICK_QUESTIONS = [
  "groupPages.ai.quickQuestions.unpaid",
  "groupPages.ai.quickQuestions.cashbox",
  "groupPages.ai.quickQuestions.largestArrears",
  "groupPages.ai.quickQuestions.monthEvolution",
  "groupPages.ai.quickQuestions.decisions",
  "groupPages.ai.quickQuestions.birthdays",
  "groupPages.ai.quickQuestions.meetings",
];
  ...
  return (
    <div className="p-6 space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-xl font-black text-[#17211f] dark:text-white">{tr("groupPages.ai.title")}</h2>
          <p className="text-xs text-[#717182]">{tr("groupPages.ai.subtitle")}</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        <button onClick={() => report.mutate("monthly")} disabled={report.isPending}
          ...
          <FileText size={12} /> {tr("groupPages.ai.monthlyReport")}
        </button>
        <button onClick={() => report.mutate("payments")} disabled={report.isPending}
          ...
          <BarChart3 size={12} /> {tr("groupPages.ai.contributionAnalysis")}
        </button>
        <button onClick={() => analysis.mutate()} disabled={analysis.isPending}
          ...
          {analysis.isPending ? <Loader2 size={12} className="animate-spin" /> : <BarChart3 size={12} />} {tr("groupPages.ai.paymentAnalysis")}
        </button>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {QUICK_QUESTIONS.map(q => (
          <button key={q} onClick={() => { ask.mutate(tr(q)); }}
      ...
      <div className="min-h-[200px] space-y-3">
        {messages.length === 0 && !ask.isPending && (
          <div className="flex flex-col items-center py-10 text-center">
            ...
            <p className="text-sm text-[#717182] max-w-sm">{tr("groupPages.ai.empty")}</p>
          </div>
        )}
        {messages.map((msg, i) => (
        ...
        {ask.isPending && (
          <div className="flex justify-start">
            <div className="rounded-2xl bg-[#ececf2] dark:bg-white/[0.07] px-4 py-3 flex items-center gap-2">
              ...
              <span className="text-sm text-[#717182]">{tr("groupPages.ai.analyzing")}</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center gap-2 rounded-xl border border-black/[0.06] dark:border-white/[0.06] bg-[#f5f6f8] dark:bg-[#252931] px-3 py-2.5">
        <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && !e.shiftKey && sendMsg()}
          placeholder={tr("groupPages.ai.placeholder")}
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 px-4 backdrop-blur-sm">
          <div className="w-full max-w-2xl rounded-2xl border border-black/[0.06] dark:border-white/[0.08] bg-white dark:bg-[#1e2229] p-6 shadow-2xl max-h-[85vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2"><LimuleIcon size={16} /><h3 className="font-black text-[#17211f] dark:text-white">{tr("groupPages.ai.generatedReport")}</h3></div>
              <button onClick={() => setShowReport(false)} className="rounded-lg bg-black/[0.05] dark:bg-white/[0.07] px-3 py-1 text-xs font-bold text-[#717182]">{tr("common.close")}</button>
            </div>
