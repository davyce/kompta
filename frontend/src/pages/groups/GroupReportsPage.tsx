...
import { TrendingDown, CreditCard, Loader2 } from "lucide-react";
import { useTranslation } from "react-i18next";
import { api } from "../../services/api";
import i18n from "../../i18n";

function fmtAmount(v: number, cur = "XAF", locale = i18n.language) {
  return new Intl.NumberFormat(locale, { style: "currency", currency: cur, minimumFractionDigits: 0 }).format(v);
}
...
export function GroupReportsPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      <h2 className="text-xl font-black text-[#17211f] dark:text-white">{tr("groupPages.reports.title")}</h2>

      {/* Rapport cotisations */}
      <div className="rounded-xl border border-black/[0.06] dark:border-white/[0.06] bg-white dark:bg-[#1e2229] p-5">
        <div className="flex items-center gap-2 mb-4"><CreditCard size={16} className="text-blue-700" /><h3 className="font-bold text-[#17211f] dark:text-white">{tr("groupPages.reports.contributions")}</h3></div>
          ...
          <>
            <div className="grid grid-cols-3 gap-3 mb-4">
              <div className="rounded-xl bg-[#f6f7fb] dark:bg-[#161920] p-3">
                <p className="text-xs text-[#717182]">{tr("groupPages.reports.totalDue")}</p>
                <p className="text-lg font-black text-[#17211f] dark:text-white">{fmtAmount(payReport?.total_due ?? 0, currency, i18n.language)}</p>
              </div>
              <div className="rounded-xl bg-[#f6f7fb] dark:bg-[#161920] p-3">
                <p className="text-xs text-[#717182]">{tr("groupPages.reports.totalReceived")}</p>
                <p className="text-lg font-black text-emerald-600">{fmtAmount(payReport?.total_paid ?? 0, currency, i18n.language)}</p>
              </div>
              <div className="rounded-xl bg-[#f6f7fb] dark:bg-[#161920] p-3">
                <p className="text-xs text-[#717182]">{tr("groupPages.reports.recoveryRate")}</p>
            ...
            {(payReport?.rows ?? []).filter(r => r.status !== "paid").slice(0, 5).length > 0 && (
              <div>
                <p className="text-xs font-bold uppercase text-[#717182] mb-2">{tr("groupPages.reports.membersInArrears")}</p>
                {(payReport?.rows ?? []).filter(r => r.status !== "paid").slice(0, 5).map((r, i) => (
                  <div key={i} className="flex items-center justify-between py-1.5 border-b border-black/[0.04] dark:border-white/[0.04] last:border-0">
                    ...
                    <p className="text-sm font-black text-rose-600">{fmtAmount(r.amount_due - r.amount_paid, currency, i18n.language)}</p>
                  </div>
                ))}
              </div>
            )}
      ...
      {/* Rapport dépenses */}
      <div className="rounded-xl border border-black/[0.06] dark:border-white/[0.06] bg-white dark:bg-[#1e2229] p-5">
        <div className="flex items-center gap-2 mb-4"><TrendingDown size={16} className="text-rose-500" /><h3 className="font-bold text-[#17211f] dark:text-white">{tr("groupPages.reports.expenses")}</h3></div>
        {l2 ? <div className="flex h-20 items-center justify-center"><Loader2 size={20} className="animate-spin text-rose-500" /></div> : (
          <>
            <p className="text-2xl font-black text-rose-600 mb-4">{fmtAmount(expReport?.total ?? 0, currency, i18n.language)}</p>
            {/* Par catégorie */}
            {Object.entries(expReport?.by_category ?? {}).map(([cat, amount]) => {
              ...
              return (
                <div key={cat} className="mb-2">
                  <div className="flex justify-between text-xs mb-0.5"><span className="text-[#717182] capitalize">{cat}</span><span className="font-bold">{fmtAmount(amount, currency, i18n.language)} ({pct}%)</span></div>
