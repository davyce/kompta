    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={onClose}>
      <form onClick={(e) => e.stopPropagation()} onSubmit={submit}
        ...
        <div className="flex items-center justify-between">
          <h3 className="font-bold text-[#17211f] dark:text-white">Nouvelle réunion</h3>
        ...
        <div className="grid grid-cols-2 gap-2">
          <input placeholder="Tag (Direction, RH…)" value={form.tag} onChange={(e) => setForm({ ...form, tag: e.target.value })}
            className="rounded-lg border border-black/[0.08] dark:border-white/[0.08] dark:bg-[#252931] px-3 py-2 text-sm" />
        ...
        <input placeholder="Lieu (salle, adresse…)" value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })}
          className="w-full rounded-lg border border-black/[0.08] dark:border-white/[0.08] dark:bg-[#252931] px-3 py-2 text-sm" />
        <input placeholder="Lien visio (optionnel)" value={form.join_url} onChange={(e) => setForm({ ...form, join_url: e.target.value })}
          className="w-full rounded-lg border border-black/[0.08] dark:border-white/[0.08] dark:bg-[#252931] px-3 py-2 text-sm" />
        <input placeholder="Participants (noms séparés par virgules)" value={form.attendees} onChange={(e) => setForm({ ...form, attendees: e.target.value })}
          className="w-full rounded-lg border border-black/[0.08] dark:border-white/[0.08] dark:bg-[#252931] px-3 py-2 text-sm" />
        {create.isError && <p className="text-xs text-rose-600">Impossible de créer la réunion.</p>}
        <button type="submit" disabled={create.isPending}
          ...
          {create.isPending ? "Création…" : "Créer la réunion"}
        </button>
      </form>
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-sm font-semibold text-emerald-600">Collaboration</p>
          <h1 className="text-3xl font-extrabold text-[#17211f] dark:text-white">Réunions intelligentes</h1>
          <p className="mt-1 text-sm text-[#717182]">Calendrier, ordres du jour, comptes-rendus Limule</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <button
            ...
            <Calendar size={15} /> Exporter calendrier (.ics)
          </button>
          <button
            ...
            <Plus size={15} /> Proposer une réunion
          </button>
        </div>
      </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[1fr_360px]">
        ...
        <div className="rounded-xl border border-black/[0.06] bg-white dark:bg-[#1e2229] dark:border-white/[0.06]">
          <div className="flex items-center justify-between border-b border-black/[0.05] dark:border-white/[0.05] px-5 py-4">
            <div>
              <h3 className="font-bold text-[#17211f] dark:text-white capitalize">Aujourd'hui · {today}</h3>
              <p className="text-xs text-[#717182]">{todayMeetings.length} réunion{todayMeetings.length > 1 ? "s" : ""} planifiée{todayMeetings.length > 1 ? "s" : ""}</p>
            </div>
          ...
          <div className="divide-y divide-black/[0.04] dark:divide-white/[0.04]">
            {meetings.isLoading && <p className="px-5 py-8 text-sm text-[#717182]">Chargement…</p>}
            {!meetings.isLoading && todayMeetings.length === 0 && (
              <div className="px-5 py-12 text-center">
                <Calendar size={32} className="mx-auto text-[#717182] opacity-40" />
                <p className="mt-3 text-sm text-[#717182]">Aucune réunion aujourd'hui.</p>
                <button onClick={() => setShowCreate(true)} className="mt-3 text-sm font-semibold text-emerald-600 hover:text-emerald-700">+ Programmer une réunion</button>
              </div>
            )}
            {todayMeetings.map((m) => {
              ...
              return (
                ...
                >
                  <div className="flex w-12 flex-shrink-0 flex-col items-center rounded-xl border border-black/[0.06] bg-white dark:bg-[#252931] dark:border-white/[0.06] py-2 shadow-sm">
                    <span className="text-[11px] font-bold uppercase text-emerald-600">{day.toLocaleDateString("fr-FR", { month: "short" }).toUpperCase().slice(0, 3)}</span>
        ...
        <div className="space-y-4">
          ...
          {lastSummary ? (
            <div className="rounded-xl border border-black/[0.06] bg-white dark:bg-[#1e2229] dark:border-white/[0.06]">
              <div className="border-b border-black/[0.05] dark:border-white/[0.05] px-5 py-4">
                <h3 className="font-bold text-[#17211f] dark:text-white">Compte-rendu Limule — {lastSummary.title}</h3>
                <p className="text-xs text-[#717182]">{new Date(lastSummary.start_at).toLocaleString("fr-FR", { dateStyle: "medium", timeStyle: "short" })}</p>
              </div>
              <div className="px-5 py-4 space-y-3">
                ...
                <button
                  ...
                  {convertToTasks.isPending ? "Conversion…" : `Convertir en ${lastSummary.ai_points.length} tâches →`}
                </button>
              </div>
            </div>
          ) : (
            <div className="rounded-xl border border-dashed border-black/[0.1] dark:border-white/[0.08] px-5 py-8 text-center">
              ...
              <p className="mt-2 text-sm font-semibold text-[#17211f] dark:text-white">Aucun compte-rendu Limule</p>
              <p className="text-xs text-[#717182]">Termine une réunion et génère son résumé.</p>
            </div>
          )}
            ...
            <div className="rounded-xl border border-emerald-200 bg-emerald-50 dark:bg-emerald-500/10 dark:border-emerald-500/30 px-5 py-4 space-y-2">
              ...
              {!selectedMeeting.ai_summary && (
                <button
                  ...
                  {generateSummary.isPending ? "Génération Limule…" : "Générer un résumé Limule"}
                </button>
              )}
            </div>
          )}
          ...
          <div className="rounded-xl border border-black/[0.06] bg-white dark:bg-[#1e2229] dark:border-white/[0.06]">
            <div className="border-b border-black/[0.05] dark:border-white/[0.05] px-5 py-4">
              <h3 className="font-bold text-[#17211f] dark:text-white">Prochaines réunions</h3>
            </div>
            <div className="divide-y divide-black/[0.04] dark:divide-white/[0.04]">
              {upcoming.length === 0 && (
                <p className="px-5 py-6 text-sm text-[#717182]">Rien à venir.</p>
              )}
              {upcoming.map((u) => (
                <button
                  key={u.id}
