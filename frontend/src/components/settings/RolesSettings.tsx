...
export function RolesSettings() {
  ...
  return (
    <div className="space-y-8">
      {/* En-tête */}
      <div>
        <h2 className="text-lg font-black text-[#17211f] dark:text-white">Rôles & accès</h2>
        <p className="mt-1 text-sm text-[#717182] dark:text-white/60">
          Créez des rôles avec un accès limité à certains modules, puis attribuez-les à vos membres.
          Le rôle d'un membre détermine les modules qu'il voit dans l'application.
        </p>
      </div>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-bold uppercase tracking-wider text-slate-500 dark:text-white/50">Rôles personnalisés</h3>
          <button
        ...
        {(roles.data ?? []).length === 0 && !roles.isLoading && (
          <div className="rounded-xl border border-dashed border-slate-300 p-6 text-center text-sm text-slate-400 dark:border-white/15 dark:text-white/40">
            Aucun rôle personnalisé. Créez-en un pour limiter l'accès de certains membres.
          </div>
        )}
            ...
            <div key={r.id} className="flex items-start gap-3 rounded-xl border border-slate-200 bg-white p-4 dark:border-white/10 dark:bg-white/5">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-[11px] font-semibold text-slate-400 dark:text-white/40">
                  {r.permissions.length} accès · {r.member_count} membre(s)
                </p>
              </div>
              <div className="flex shrink-0 gap-1">
                <button onClick={() => { setCreating(false); setEditing(r); }} className="grid h-7 w-7 place-items-center rounded-md text-slate-500 hover:bg-black/[0.05] dark:text-white/60 dark:hover:bg-white/10">
                  <Pencil size={14} />
                </button>
                <button
                  onClick={() => { if (confirm(`Supprimer le rôle « ${r.name} » ?`)) deleteRole.mutate(r.id); }}
        ...
        <div className="divide-y divide-slate-100 overflow-hidden rounded-xl border border-slate-200 dark:divide-white/5 dark:border-white/10">
          {(users.data ?? []).map((u) => (
            <div key={u.id} className="flex items-center gap-3 bg-white px-4 py-3 dark:bg-white/5">
              ...
              >
                <option value="">Rôle de base ({u.role})</option>
                {(roles.data ?? []).map((r) => (
          ...
          {(users.data ?? []).length === 0 && !users.isLoading && (
            <div className="bg-white px-4 py-6 text-center text-sm text-slate-400 dark:bg-white/5 dark:text-white/40">Aucun membre.</div>
          )}
        </div>
      </section>
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50 p-4" role="dialog" aria-modal="true">
      <div className="flex max-h-[88vh] w-full max-w-lg flex-col overflow-hidden rounded-2xl bg-white shadow-2xl dark:bg-[#1a1d23]">
        <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 dark:border-white/10">
          <h3 className="font-black text-slate-900 dark:text-white">{role ? "Modifier le rôle" : "Nouveau rôle"}</h3>
          <button onClick={onClose} className="grid h-7 w-7 place-items-center rounded-lg text-slate-500 hover:bg-black/[0.05] dark:text-white/60 dark:hover:bg-white/10"><X size={16} /></button>
        </div>

        <div className="flex-1 space-y-4 overflow-y-auto px-5 py-4">
          <div>
            <label className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-white/50">Nom du rôle</label>
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Comptable, Caissier…"
          ...
          <div>
            <label className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-white/50">Description</label>
            <input value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Optionnel"
              className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-900 outline-none focus:border-emerald-400 dark:border-white/10 dark:bg-black/20 dark:text-white" />
          </div>
          <div>
            <label className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-white/50">Couleur</label>
        ...
        <div className="flex items-center justify-end gap-2 border-t border-slate-100 px-5 py-3 dark:border-white/10">
          <button onClick={onClose} className="rounded-lg px-3 py-2 text-sm font-semibold text-slate-500 hover:bg-black/[0.05] dark:text-white/60 dark:hover:bg-white/10">Annuler</button>
          <button onClick={() => save.mutate()} disabled={!canSave || save.isPending}
            className="rounded-lg bg-emerald-600 px-4 py-2 text-sm font-bold text-white hover:bg-emerald-700 disabled:opacity-50">
            {save.isPending ? "Enregistrement…" : role ? "Enregistrer" : "Créer le rôle"}
          </button>
        </div>
      </div>
    </div>
