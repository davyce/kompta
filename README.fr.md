<p align="center"><img src="docs/assets/logo.png" width="112" alt="Logo KOMPTA" /></p>
...
# ── IA / Limule (propulsé par LIMULEIA) ─────────────────────────
DEEPSEEK_API_KEY=sk-xxxxxxxxxxxxxxxx      # fournisseur de modèle derrière LIMULEIA, voir github.com/davyce/limuleia
...
- Génération de contrat via IA (Limule, propulsé par LIMULEIA) + stockage Documents
...
### v1.14.0 — Juillet 2026 (accessibilité web + iOS/Mac, i18n web, correctifs audit)

- ✅ **Accessibilité web (WCAG)** : remédiation systématique sur l'ensemble des pages web — ~50 fichiers, 8 lots — noms accessibles (`aria-label`) ajoutés sur les contrôles icône-seule, associations `<label htmlFor>`/`id` sur les champs de formulaire, `role="switch"`/`aria-pressed`/`aria-checked` sur les bascules, actions révélées au survol rendues aussi visibles au focus clavier (`focus-within`/`focus-visible`). Quatre `<div onClick>` non accessibles (zone de dépôt de fichier, bascules "preuve requise"/"obligatoire", carte de note) transformés en contrôles natifs `role="button"`/`role="switch"` avec support clavier complet (Entrée/Espace)
- ✅ **i18n web** : migration des derniers composants en français codé en dur vers le système `react-i18next` — l'assistant de configuration d'entreprise (25 étapes d'onboarding), les rôles & accès personnalisés, le panneau Stripe Connect, les méthodes d'encaissement, et quelques chaînes orphelines dans Paramètres/Clients. Le contenu marketing (landing page) et légal, ainsi que le tableau réglementaire OHADA des déclarations, restent volontairement en français (décision produit, pas une lacune technique)
- ✅ **VoiceOver iOS/Mac** : ajout de `.accessibilityLabel` sur la cloche de notifications partagée (`NotificationBell`, présente sur tous les écrans), le bouton panier et le bouton d'effacement de la recherche en caisse, et 46 boutons icône-seule répartis sur 16 fichiers de vues (Admin, Collaboration, Groupes, RH, Finance, Achats, Investissements, CRM, Inventaire, Rôles, assistant de configuration). Vérifié par build complet iOS (simulateur) et macOS
- ✅ **Correctifs ciblés issus de l'audit qualité** : health-check IA mis en cache côté serveur (TTL 2 min) pour éliminer un blocage de 27 s au premier chargement, débordement horizontal de la grille Paie sur mobile corrigé, superposition de l'assistant de configuration et du tour guidé au premier chargement corrigée (état partagé entre les deux composants), nettoyage (référence Xcode fantôme, artefacts de build Vite, composant inutilisé, lien Anniversaires manquant dans la navigation Groupes)
