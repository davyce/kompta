...
struct POSView: View {
            ...
            Button {
                ...
                HStack {
                    if isSaving {
                        ...
                        Image(systemName: paymentMethod == "tap_to_pay" ? "wave.3.right.circle.fill" : "checkmark.seal.fill")
                        Text(paymentMethod == "tap_to_pay" ? "Tap to Pay · \(fcfa(grandTotal))" : "Encaisser · \(fcfa(grandTotal))")
                    }
                }
                .frame(maxWidth: .infinity)
                .padding(.vertical, 14)
    ...
    /// Encaissement Tap to Pay on iPhone : la carte doit être débitée AVANT
    /// que la vente ne soit créée côté backend.
